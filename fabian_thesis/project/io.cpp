...
char* usage(char *argv[], char *usage_msg){
	(void)sprintf(usage_msg, "Usage: %s -v variables -c cores -i implementation [-r revisions] [-f file] [-t threads]", argv[0]);
	return usage_msg;
}
...
void process_args(int argc, char *argv[], io_info *info){
	...
	if (!(v_opt && c_opt && i_opt) || (argc != opt_cnt))
		error_exit(1, usage(argv, usage_str));
...
void process_output(io_info *info){
	...
	fprintf(info->f, "%d,%d,%.2f,%d\n", info->v_opt, info->c_opt, average, info->i_opt);
	fflush(info->f);
	fclose(info->f);
}